...
model InputConflictBeneBaB
  ...
  PNlib.Components.PD P1(benefitType = PNlib.Types.BenefitType.BranchAndBound, enablingBeneIn = {6, 5}, enablingType = PNlib.Types.EnablingType.Benefit, maxTokens = 5, nIn = 2, nOut = 1) annotation(
    Placement(transformation(extent = {{10, -10}, {30, 10}})));
  PNlib.Components.TD T1(arcWeightIn = {3}, nIn = 1) annotation(
    Placement(visible = true, transformation(extent = {{40, -10}, {60, 10}}, rotation = 0)));
  PNlib.Components.TD T2(arcWeightOut = {3}, nIn = 1, nOut = 1) annotation(
    Placement(transformation(extent = {{-20, 10}, {0, 30}})));
  PNlib.Components.TD T3(arcWeightOut = {2}, nIn = 1, nOut = 1) annotation(
    Placement(transformation(extent = {{-20, -30}, {0, -10}})));
  PNlib.Components.PD P2(startTokens = 5, nOut = 1) annotation(
    Placement(transformation(extent = {{-50, 10}, {-30, 30}})));
  PNlib.Components.PD P3(startTokens = 5, nOut = 1) annotation(
...
equation
  connect(T1.inPlaces[1], P1.outTransition[1]) annotation(
    Line(points = {{45, 0}, {30.8, 0}}));
  connect(P2.outTransition[1], T2.inPlaces[1]) annotation(
    Line(points = {{-29.2, 20}, {-14.8, 20}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(P3.outTransition[1], T3.inPlaces[1]) annotation(
    Line(points = {{-29.2, -20}, {-14.8, -20}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(T2.outPlaces[1], P1.inTransition[1]) annotation(
    Line(points = {{-5.2, 20}, {0, 20}, {0, -0.5}, {9.2, -0.5}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(T3.outPlaces[1], P1.inTransition[2]) annotation(
    Line(points = {{-5.2, -20}, {0, -20}, {0, 0.5}, {9.2, 0.5}}, color = {0, 0, 0}, smooth = Smooth.None));
  annotation(
