...
model CTC
  ...
  Real arcWeightIn[nIn, numColors] = fill(1, nIn, numColors) "arc weights of input places" annotation(
    Dialog(enable = true, group = "Arc Weights"));
  Real arcWeightOut[nOut, numColors] = fill(1, nOut, numColors) "arc weights of output places" annotation(
    Dialog(enable = true, group = "Arc Weights"));
  PNlib.Interfaces.TransitionIn[nIn, numColors] inPlaces annotation(
    Placement(transformation(extent = {{-56, -10}, {-40, 10}})));
  PNlib.Interfaces.TransitionOut[nOut, numColors] outPlaces annotation(
    Placement(transformation(extent = {{40, -10}, {56, 10}})));
protected
...
equation
  ...
  annotation(
    Icon(graphics = {Rectangle(extent = {{-40, 100}, {40, -100}}, lineColor = {0, 0, 0}, fillColor = DynamicSelect({255, 255, 255}, color), fillPattern = FillPattern.Solid)}));
end CTC;
