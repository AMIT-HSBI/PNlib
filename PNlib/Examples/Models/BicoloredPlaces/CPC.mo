...
protected
  ...
  Components.PC place1(
    nIn=nIn,
    nOut=nOut,
    startMarks=startMarks[1],
    minMarks=minMarks[1],
    maxMarks=maxMarks[1]);
  Components.PC place2(
    nIn=nIn,
    nOut=nOut,
    startMarks=startMarks[2],
    minMarks=minMarks[2],
    maxMarks=maxMarks[2]);
equation
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio=false), graphics={Ellipse(extent={{-100,98},{100,-96}}, lineColor={0,0,0}, fillColor={255,255,255},
            fillPattern =                                                                                                                                              FillPattern.Solid), Ellipse(extent={{-88,86},{88,-86}}, lineColor={0,0,0}, fillColor=DynamicSelect({255,255,255}, color),
            fillPattern =                                                                                                                                                                                                        FillPattern.Solid), Text(extent={{-1.5,25.5},{-1.5,-21.5}}, lineColor={0,0,0}, origin={0,35}, rotation=0, textString
            =                                                                                                                                                                                                        DynamicSelect("%startMarks[1]",if animateMarking==1 then if color[1]>0 then realString(color[1],1,2) else "0.0" else " ")), Text(extent={{-1.5,25.5},{-1.5,-21.5}}, lineColor={0,0,0}, origin={0,-35}, rotation=0, textString
            =                                                                                                                                                                                                        DynamicSelect("%startMarks[2]",if animateMarking==1 then if color[2]>0 then realString(color[2],1,2) else "0.0" else " ")), Line(points={{-88,0},{88,0}}, color={0,0,0})}), Diagram(coordinateSystem(preserveAspectRatio=false)));
end CPC;
