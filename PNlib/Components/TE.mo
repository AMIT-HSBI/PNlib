...
model TE "Discrete Transition with event"
  ...
  Real event[:] = {1, 2, 3} "Event time of timed transition" annotation(
    Dialog(enable = true, group = "Event"));
...
equation
  ...
  annotation(
    ...
    Icon(graphics = {Rectangle(extent = {{-40, 100}, {40, -100}}, lineColor = {0, 0, 0}, fillColor = DynamicSelect({0, 0, 0}, color), fillPattern = FillPattern.Solid), Text(extent = {{-2, -112}, {-2, -140}}, lineColor = {0, 0, 0}, textString = DynamicSelect("e=%event", if showevent then "e=%event" else " ")), Text(extent = {{-4, 139}, {-4, 114}}, lineColor = {0, 0, 0}, textString = "%name")}),
    Diagram(graphics));
end TE;
