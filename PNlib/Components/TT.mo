...
model TT "Discrete Transition"
  ...
  parameter Real tactIntervall = 1 "tact intervall of timed transition" annotation(
    Dialog(enable = true, group = "Takt"));
  parameter Real tactStart = 1 "tact start of timed transition" annotation(
    Dialog(enable = true, group = "Takt"));
...
equation
  ...
  annotation(
    ...
    Icon(graphics = {Rectangle(extent = {{-40, 100}, {40, -100}}, lineColor = {0, 0, 0}, fillColor = DynamicSelect({0, 0, 0}, color), fillPattern = FillPattern.Solid), Text(extent = {{-2, -112}, {-2, -140}}, lineColor = {0, 0, 0}, textString = DynamicSelect("ti=%tactIntervall", if showTakt == 1 then "ti=%tactIntervall" else " ")), Text(extent = {{-2, -152}, {-2, -180}}, lineColor = {0, 0, 0}, textString = DynamicSelect("ts=%tactStart", if showTakt == 1 then "ts=%tactStart" else " ")), Text(extent = {{-4, 139}, {-4, 114}}, lineColor = {0, 0, 0}, textString = "%name")}),
    Diagram(graphics));
end TT;
