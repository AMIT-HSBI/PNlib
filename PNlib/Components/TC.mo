...
equation
  ...
  annotation(
    ...
    Icon(graphics = {Rectangle(extent = {{-40, 100}, {40, -100}}, lineColor = {0, 0, 0}, fillColor = DynamicSelect({255, 255, 255}, color), fillPattern = FillPattern.Solid), Text(extent = {{-2, -116}, {-2, -144}}, lineColor = {0, 0, 0}, textString = DynamicSelect("vmax=%maximumSpeed ", if animateSpeed then "vmax=%maximumSpeed " else " ")), Text(extent = {{-2, -156}, {-2, -184}}, lineColor = {0, 0, 0}, textString = DynamicSelect(" ", if animateSpeed and fire then if instantaneousSpeed > 0 then "vakt=" + String(instantaneousSpeed) else "vakt=0.0" else " ")), Text(extent = {{-4, 139}, {-4, 114}}, lineColor = {0, 0, 0}, textString = "%name")}));
end TC;
