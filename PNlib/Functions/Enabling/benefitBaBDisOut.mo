...
algorithm
  ...
  if not TAout[Index] or not disTransition[Index] then
    (TEoutOut, TokenOut, benefitMaxOut, valid, benefitLimitOut) := benefitBaBDisOut(depth + 1, nOut, benefit, arcWeight, Quotient_, t, benefitMaxIn - benefit[Index], minTokens, TEoutOut, TokenOut, benefitLimit, TAout, disTransition);
    return;
  end if;
  // partial solution 1
  ...
  if depth < nOut then // no finished solution
    if t - TokenOut1 >= minTokens and benefitMaxOut1 >= benefitLimit then
      (TEoutOut1, TokenOut1, benefitMaxOut1, valid1, benefitLimit) := benefitBaBDisOut(depth + 1, nOut, benefit, arcWeight, Quotient_, t, benefitMaxOut1, minTokens, TEoutOut1, TokenOut1, benefitLimit, TAout, disTransition);
    else
      valid1 := false;
    end if;
    if t - TokenOut2 >= minTokens and benefitMaxOut2 >= benefitLimit then
      (TEoutOut2, TokenOut2, benefitMaxOut2, valid2, benefitLimit) := benefitBaBDisOut(depth + 1, nOut, benefit, arcWeight, Quotient_, t, benefitMaxOut2, minTokens, TEoutOut2, TokenOut2, benefitLimit, TAout, disTransition);
    else
      valid2 := false;
    end if;
